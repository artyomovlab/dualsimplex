...
plot_numeric_features(lo2$get_data(), features = c("log_mad", "log_mean", "log_median", "log_sd"), ncol = 4)
```

```{r}
# TODO: before and after filtering plots, more explicit noise removal
# svd_d <- diag(lo2$st$proj$meta$Sigma)
...
```{r}
plot_feature(lo2$st$data, "log_mad")
lo2$plot_svd()
lo2$set_data(lo2$st$data[fData(lo2$st$data)$log_mad > 1, ])
plot_feature(lo2$st$data, "log_mad")
lo2$plot_svd()
```
...
lo2$plot_projected("zero_distance", "zero_distance", use_dims = list(2:3, 3:4, 2:4))
lo2$plot_projected("plane_distance", "plane_distance", use_dims = list(2:3, 3:4, 2:4))
```
...
lo2$run_umap(neighbors_X = 50, neighbors_Omega = 25)
lo2$plot_projected("zero_distance", "zero_distance")
lo2$plot_projected("plane_distance", "plane_distance")
```

### Distance-based filtering
```{r, fig.height=4, fig.width=10}
plot_numeric_features(lo2$get_data(), features = c("plane_distance", "zero_distance"), ncol = 2)
plot_numeric_features(lo2$get_data(), genes = F, features = c("plane_distance", "zero_distance"), ncol = 2, bins = 30)
```

```{r}
new_data <- lo2$st$data[fData(lo2$get_data())$plane_distance < 0.1, ]
new_data
```

```{r}
lo2$set_data(new_data)
```
...
lo2$set_display_dims(list(NULL, 3:4))
```
...
```{r}
lo2$save_state("./linseed2_save")
```


## Perform deconvolution
```{r}
lo2 <- Linseed2Solver$from_state("./linseed2_save")
...
lo2$optim_solution(1000, optim_config(coef_hinge_H = 0.1))
...
lo2$plot_projected(NULL, NULL, pt_opacity = 1, pt_size = 2)
```
...
plot_projection_points(lo2$st$proj, pt_opacity = 0.8, pt_size = 0.1) %>% 
  ...
  add_solution_history(lo2$st$solution_proj, lo2$st$proj, pt_size = 1.5, pt_opacity = 1, step = 20, colored = T)
```
...
```{r}
# Note: unlike Linseed2Solver$from_state, the load_state method doesn't create 
...
lo2$generate_summary("abc")
```

## Comparing with true data
When true solution is available, we can compare linseed with it
```{r}
lo2 <- Linseed2Solver$from_state("./linseed2_save")
...
```{r}
# Note: it's a separate function, not inside lo2
