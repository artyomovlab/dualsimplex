...
```{r}
save_dir <- "./dualsimplex_save_gtex_lung_11kg_8ct"
```

```{r}
# Fresh object (for the first run)
dso <- DualSimplexSolver$new()
dso$set_save_dir(save_dir)
```

```{r}
# Load from checkpoint option (to continue a previous run)
dso <- DualSimplexSolver$from_state(save_dir, save_there = T)
```

## Set data
```{r}
# To calculate this step faster, reduce the dataset size to 10-20k genes
dso$set_data(data_raw, gene_anno_lists = list(
...
dso$plot_projected("zero_distance", "zero_distance", use_dims = all_dims)
dso$plot_projected("plane_distance", "plane_distance", use_dims = all_dims)
dso$plot_distances_distribution()
```

```{r}
# Uncomment
# dso$distance_filter(plane_d_lt = 0.05, zero_d_lt = NULL, genes = T)
```
...
dso$run_umap(neighbors_X = 40, neighbors_Omega = 40)
...
dso$set_display_dims(display_dims)
```
...
dso$optim_solution(2000, optim_config(coef_hinge_H = 0.01, coef_hinge_W = 0.1))
...
```{r fig.height = 5, fig.width = 11}
dso$plot_projected(use_dims = all_dims)
```
...
plot_projection_points(dso$st$proj, pt_opacity = 0.8, pt_size = 0.1) %>%
  ...
  add_solution_history(dso$st$solution_proj, dso$st$proj, pt_size = 0.5, pt_opacity = 1, step = 20, colored = T)
```
...
```{r}
dso$set_display_dims(list(2:3))
dso$generate_summary(seurat_obj = seurat_obj, with_animated_optim = FALSE)
```


# Manual additional research and figure fine-tuning
```{r fig.height = 5, fig.width = 11}
dso$plot_projected(with_legend = T)
```
```{r}
seurat_obj <- add_list_markers(seurat_obj, dso$get_marker_genes())
dso$get_marker_genes()$cell_type_1 %in% rownames(seurat_obj)
nrow(seurat_obj)
```
...
```{r fig.height = 5, fig.width = 11}
dso$st$marker_genes <- get_signature_markers(dso$st$solution$W, 20)
dso$plot_projected("markers", use_dims = all_dims)
dso$plot_projected("markers", use_dims = NULL)
```
