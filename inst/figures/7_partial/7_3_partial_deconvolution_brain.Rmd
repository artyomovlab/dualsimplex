...
marker_labels <- rep(NA, length(rownames(genes_umap)))
lab <- marker_labels
names(marker_labels) <- rownames(genes_umap)
marker_labels[names(rev_mlist)] <- rev_mlist
marker_labels <- unlist(marker_labels[rownames(genes_umap)])

to_plot <- as.data.frame(genes_umap)
to_plot[["cell_type"]] <- brain_ct_map_long[marker_labels]
ggplot(to_plot[is.na(lab),], aes_string(x="V2", y="V1", col="cell_type")) +
    geom_point() +
