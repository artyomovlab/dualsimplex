...
Rcpp::List markers_derivative_stage2(const arma::mat& X,
                             ...
                             const double thresh,
                             const double center_constraint
                            ) {
    ...
    for (int itr_ = 0; itr_ < iterations; itr_++) {
        ...
        if (center_constraint > 0) {
            // Here all the job starts. We always expect point is already within the center constraint
            ...
            for (int c=0; c < cell_types; c++) {
                ...
                if (!(X_center.row(c).subvec(1, cell_types - 1).is_zero())) {
                    ...
                    // First calculate  distance between given row of new X and center (for dimensions 2:K)
                    double fro_distance = arma::norm(tmp_X.row(c).subvec(1, cell_types - 1)-X_center.row(c).subvec(1, cell_types - 1), "fro");
                    if (fro_distance > center_constraint) {
                        // we are to farr away.
                        // start shrinking derivative to be inside. Just simply divide it by 2 util convergence.
                        int shrink_iteration = 0;
                        while(fro_distance > center_constraint) {
                            der_X.row(c) /=  2;
                            tmp_X = (new_X - coef_der_X * der_X);
                            fro_distance = arma::norm(tmp_X.row(c).subvec(1, cell_types - 1) - X_center.row(c).subvec(1, cell_types - 1));
