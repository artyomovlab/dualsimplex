...
reverse_sinkhorn_2 <- function(H_row, W_col, scaling) {
  #' Calculate all normalizing matrices and restore "unscaled" W and H
  #'
  #' columns of D_ws_col and D_hs_row will
  #' contain history of D_w and D_h matrices
  ...
  unscaled <- reverse_sinkhorn_c_2(H_row,
                                 W_col,
                                 scaling$D_vs_row,
                                 scaling$D_vs_col,
                                 scaling$iterations)
...
reverse_solution_sinkhorn_2 <- function(solution_scaled, scaling) {
  return(reverse_sinkhorn_2(
    solution_scaled$H_row,
    solution_scaled$W_col,
    scaling
  ))
